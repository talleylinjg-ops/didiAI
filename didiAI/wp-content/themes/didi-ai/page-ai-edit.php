<?php
/**
 * AI 剪辑工作台（视频 / 动漫 / 图片 / 音频）
 * 支持上传原始素材 + 选择编辑模型 + 写编辑指令 + 预览结果
 * 视频/动漫走本站服务；图片/音频对接用户自部署的 VPS 剪辑服务（USER_VPS_BASE_URL）
 * Template Name: AI 剪辑工作台
 */
get_header();
?>
<div class="ai-page gen" style="padding-top:0;">
  <div class="edit-tabs" style="display:flex;gap:6px;margin:2px 0 4px;flex-wrap:wrap;">
  ...
  <div class="gen-shell">
    <div class="gen-controls">
      <div class="card">
        <div id="chatcutPanel" style="display:none;margin-bottom:16px;border:1px solid #5eead4;border-radius:12px;padding:16px;background:#f0fdfa;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
            <p style="font-size:15px;font-weight:700;color:#0d9488;margin:0;">ChatCut 智能剪辑</p>
            <span class="badge success" id="chatcutStatusBadge">未连接</span>
          </div>
          <div id="chatcutDisconnected">
            <button type="button" class="btn" id="btn-connect-chatcut" style="width:100%;box-sizing:border-box;justify-content:center;border-color:#14b8a6;color:#0d9488;">授权连接 ChatCut 账号</button>
          </div>
          <div id="chatcutConnected" style="display:none;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
              ...
              <button type="button" class="btn" id="btn-disconnect-chatcut" style="border-color:#e5e6eb;color:#86909c;">断开连接</button>
            </div>
            ...
            <p style="font-size:12px;color:#0f766e;margin-top:6px;">剪辑方式：对话 / 语音 / 图片 / 文件上传</p>
          </div>
        </div>
        <label class="field prompt-embedded"><span>剪辑指令</span>
        ...
        <div id="chatcutModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:400;align-items:center;justify-content:center;">
          <div style="position:relative;background:#fff;border-radius:16px;padding:28px;width:360px;max-width:92vw;text-align:center;">
            ...
            <h3 style="font-size:17px;margin:0 0 6px;">授权连接 ChatCut 账号</h3>
            <p style="font-size:13px;color:#86909c;line-height:1.7;margin-bottom:18px;">登录并授权后即可在视频剪辑中使用 ChatCut 的 52 个剪辑工具，以及对话 / 语音 / 图片 / 文件上传四种剪辑方式</p>
        ...
        <div id="chatcutHint" class="hint" style="display:none;"></div>
...
(function(){
  ...
  function saveEditModel() {
    try {
      localStorage.setItem(editModelKey(), provSel.value);
      if (provSel.value === 'custom') localStorage.setItem(editModelKey() + '__custom', editCustomInput.value.trim());
    } catch (e) {}
  }
  function hasEditOption(v) {
    for (var i = 0; i < provSel.options.length; i++) if (provSel.options[i].value === v) return true;
    return false;
  }
  function restoreEditModel() {
    var v = '', cname = '';
    try {
      v = localStorage.getItem(editModelKey()) || '';
      cname = localStorage.getItem(editModelKey() + '__custom') || '';
    } catch (e) {}
    if (v === 'custom') {
      if (cname && hasEditOption('custom')) {
        provSel.value = 'custom';
        editCustomInput.value = cname;
      }
    } else if (v && hasEditOption(v)) {
      provSel.value = v;
    } else if (v && hasEditOption('custom')) {
      provSel.value = 'custom';
      editCustomInput.value = v;
    }
    syncCustomField();
  }
  ...
  function syncChatcutPanel(){
    ...
    panel.style.display = isVideo ? '' : 'none';
    if (!isVideo) return;
    ...
    var badge = document.getElementById('chatcutStatusBadge');
    if (badge) {
      badge.textContent = conn ? '已连接' : '未连接';
      badge.className = 'badge ' + (conn ? 'success' : '');
    }
  ...
  if (googleBtn) {
    googleBtn.addEventListener('click', function () {
      ...
      document.getElementById('chatcutModal').style.display = 'none';
      syncChatcutPanel();
      toast('ChatCut 账号已连接，可在视频剪辑中使用 52 个剪辑工具及四种剪辑方式', 'success');
    });
  }
  var disconnectBtn = document.getElementById('btn-disconnect-chatcut');
  if (disconnectBtn) {
    disconnectBtn.addEventListener('click', function () {
      localStorage.setItem('chatcut_connected', '0');
      syncChatcutPanel();
      toast('已断开 ChatCut 连接', 'info');
    });
  }
