    ...
    if (kind === 'voice') {
      btn.addEventListener('click', () => {
        ...
        recog.onerror = (e) => {
          ...
          const code = e.error || '';
          const msgMap = {
            'not-allowed': '未获得麦克风权限：请在浏览器地址栏允许本站使用麦克风后重试',
            'service-not-allowed': '浏览器禁用了语音服务，请检查浏览器设置',
            'audio-capture': '未检测到可用麦克风或麦克风被占用，请检查设备后重试',
            'no-speech': '没有听到声音，请靠近麦克风再说一遍',
            'network': '语音识别服务不可用，请检查网络后重试',
            'aborted': '语音识别已取消'
          };
          toast(msgMap[code] || ('语音识别失败：' + code), 'error');
        };
      ...
      imgInput.addEventListener('change', () => {
        ...
        if (f.size > 20 * 1024 * 1024) {
          if (window.toast) toast('图片不能超过 20MB，请压缩后再试', 'error');
          imgInput.value = '';
          return;
        }
...
var DIDI_MODEL_PREFIX = 'didi_model_v1_';

function didiRememberModel(key, selectEl, opts) {
  if (!key || !selectEl) return null;
  opts = opts || {};
  var storeKey = DIDI_MODEL_PREFIX + key;
  var customStoreKey = storeKey + '__custom';
  var customValue = opts.customValue || 'custom';
  var onChange = opts.onChange || function () {};
  var ensure = opts.ensureCustomInput || function () {};
  var getInput = opts.getCustomInput || function () { return opts.customInput || null; };

  function store(v, name) {
    try {
      localStorage.setItem(storeKey, v || '');
      if (v === customValue && name) localStorage.setItem(customStoreKey, name);
    } catch (e) {}
  }
  function storedValue() { try { return localStorage.getItem(storeKey) || ''; } catch (e) { return ''; } }
  function storedCustom() { try { return localStorage.getItem(customStoreKey) || ''; } catch (e) { return ''; } }
  ...
  function apply() {
    ...
    var el = getInput();
    if (el) el.style.display = (selectEl.value === customValue) ? '' : 'none';
    onChange();
  }
  function restore() {
    var v = storedValue();
  ...
  function bindInput() {
    ...
    var el = getInput();
    if (!el || el.getAttribute('data-didi-model-bound') === '1') return;
    el.setAttribute('data-didi-model-bound', '1');
    var save = function () { store(selectEl.value, el.value.trim()); };
    el.addEventListener('input', save);
    el.addEventListener('change', save);
  }
  bindInput();
  restore();
  return { restore: function () { bindInput(); restore(); }, save: function () { store(selectEl.value, currentInput()); } };
}
