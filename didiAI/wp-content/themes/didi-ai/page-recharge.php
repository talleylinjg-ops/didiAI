...
(function () {
  ...
  amounts.forEach(function (el) {
    el.addEventListener('click', function () {
      ...
      if (selectedAmount === 0 && input) { input.focus(); }
    });
  });
  var customAmountInput = document.getElementById('custom-amount');
  if (customAmountInput) {
    customAmountInput.addEventListener('focus', function () { customAmountInput.placeholder = ''; });
    customAmountInput.addEventListener('blur', function () {
      if (!customAmountInput.value) { customAmountInput.placeholder = '自定义金额'; }
    });
  }
