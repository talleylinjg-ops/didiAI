 ...
 * Rules Engine
 *
 * @package GamiPress\Brizy\Rules_Engine
 * @since 1.0.0
 */
 ...
 * @return bool
 */
function gamipress_brizy_check_if_meets_requirements( $requirement_id, $trigger, $args ) {

    // Initialize the return value
    $return = true;

    // If is specific form trigger, rules engine needs the attached form
    if( $trigger === 'gamipress_brizy_specific_new_form_submission'
        || $trigger === 'gamipress_brizy_specific_field_value_submission' ) {

        $form_id = $args[0];
        $required_form_id = get_post_meta( $requirement_id, '_gamipress_brizy_form', true );
    ...
    if( $return && ( $trigger === 'gamipress_brizy_field_value_submission'
            || $trigger === 'gamipress_brizy_specific_field_value_submission' ) ) {

        $field_name = absint( $args[2] );
        $field_value = $args[3];

        $required_field_name = absint( gamipress_get_post_meta( $requirement_id, '_gamipress_brizy_field_name', true ) );
        $required_field_value = gamipress_get_post_meta( $requirement_id, '_gamipress_brizy_field_value', true );
 ...
 * @return array
 */
function gamipress_brizy_filter_triggered_requirements( $triggered_requirements, $user_id, $trigger, $site_id, $args ) {
    ...
    foreach( $triggered_requirements as $i => $requirement ) {
        ...
        // Skip item
        if( ! gamipress_brizy_check_if_meets_requirements( $requirement->ID, $trigger, $args ) ) {
            continue;
        }
...
add_filter( 'gamipress_get_triggered_requirements', 'gamipress_brizy_filter_triggered_requirements', 20, 5 );
 ...
 */
function gamipress_brizy_user_has_access_to_achievement( $return = false, $user_id = 0, $requirement_id = 0, $trigger = '', $site_id = 0, $args = array() ) {
    ...
    return gamipress_brizy_check_if_meets_requirements( $requirement_id, $trigger, $args );

}
add_filter( 'user_has_access_to_achievement', 'gamipress_brizy_user_has_access_to_achievement', 10, 6 );