...
return;var select2_args={theme:'default gamipress-select2',placeholder:($this.data('placeholder')?$this.data('placeholder'):gamipress_admin_functions.selector_placeholder),allowClear:!0,multiple:($this[0].hasAttribute('multiple')),matcher:gamipress_select2_words_matcher,};$this.gamipress_select2(select2_args)}
function gamipress_post_selector($this){if($this.closest('#available-widgets').length)
return;var select2_args={ajax:{url:ajaxurl,dataType:'json',delay:250,type:'POST',data:function(params){return{q:params.term,page:params.page||1,action:($this.data('action')?$this.data('action'):'gamipress_get_posts'),nonce:gamipress_admin_functions.nonce,post_type:$this.data('post-type').split(','),}},processResults:gamipress_select2_posts_process_results},escapeMarkup:function(markup){return markup},templateResult:gamipress_select2_posts_template_result,theme:'default gamipress-select2',placeholder:($this.data('placeholder')?$this.data('placeholder'):gamipress_admin_functions.post_selector_placeholder),allowClear:!0,multiple:($this[0].hasAttribute('multiple')),matcher:gamipress_select2_words_matcher,};$this.gamipress_select2(select2_args)}
...
function gamipress_select2_posts_template_result(item){if(item.post_parent_title!==undefined){return gamipress_select2_post_parent_template_result(item)}
if(item.post_type!==undefined){var post_type_label=item.post_type;if(gamipress_post_type_exists(item.post_type)){post_type_label=gamipress_get_post_type_label(item.post_type)}
...
return item.text}
function gamipress_select2_post_parent_template_result(item){if(item.post_parent_type!==undefined){var post_type_label=item.post_parent_type;if(item.post_parent_type==='points-type'){post_type_label=''}else if(gamipress_post_type_exists(item.post_parent_type)){post_type_label=gamipress_get_post_type_label(item.post_parent_type);post_type_label+=':'}
return '<strong>'+item.post_title+'</strong>'+'<span class="result-description">'+'ID: '+item.ID+'<span class="align-right">'+post_type_label+' '+item.post_parent_title+'</span>'+(item.site_name!==undefined?' ('+item.site_name+')':'')+'</span>'}
return item.text}
...
function gamipress_matches_by_word(text,term){text=text.toUpperCase();var words=term.toUpperCase().split(" ");var found=!0;for(var i=0;i<words.length;i++)
found=(found&&text.indexOf(words[i])>=0);return found}
function gamipress_select2_words_matcher(params,data){if(params.term===undefined||params.term.trim()==='')return data;if(gamipress_matches_by_word(data.text,params.term))return data;return null}
function gamipress_select2_optgroup_matcher(params,data){data.parentText=data.parentText||'';if(params.term===undefined||params.term.trim()==='')return data;if(data.children&&data.children.length>0){var match=jQuery.extend(!0,{},data);for(var c=data.children.length-1;c>=0;c--){var child=data.children[c];child.parentText+=data.parentText+" "+data.text;var matches=gamipress_select2_optgroup_matcher(params,child);if(matches==null){match.children.splice(c,1)}}
if(match.children.length>0){return match}
return gamipress_select2_optgroup_matcher(params,match)}
if(gamipress_matches_by_word((data.parentText+' '+data.text),params.term))return data;return null}
